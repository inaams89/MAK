...
const ServiceArea = ({service}) => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await fetch(`${serverUrl}service-areas`);
        if (!response.ok) {
...
<meta name="twitter:image" content={metadata.twitter.images} />
    <div className="font-sans bg-gray-100">
      <Header />
            <div className="relative bg-cover bg-center h-80" style={{ backgroundImage: 'url(/path-to-your-banner-image.jpg)' }}>
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 ">
          {services?.map((service) => (
            <Link key={service.slug} href={`/areas-covered/${service.slug}`} >
            <div key={service.id} className="p-6 bg-white   rounded-lg   shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out">
              ...
              <H5 className="text-xl font-semibold text-gray-800 hover:text-blue-600 transition-colors duration-300">{service.area}</H5>
